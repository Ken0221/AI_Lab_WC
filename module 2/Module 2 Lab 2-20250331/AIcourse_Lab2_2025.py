...
if __name__ == '__main__':
    ...
    ROOT_DIR = 'C:/Users/kenla/Desktop/113-2/AI_Lab_WC/Module_2_Lab_2-20250331'
...
#%%
    ...
    #your code
    combined = list(zip(train_dataset, training_label))
    random.shuffle(combined)
    train_dataset, training_label = zip(*combined)
...
#%%
    ...
    model = tf.keras.Sequential([
        tf.keras.layers.Conv1D(filters=32, kernel_size=3, activation='leaky_relu', padding='same', input_shape=(56,4)),
        tf.keras.layers.MaxPooling1D(pool_size=2),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),
        tf.keras.layers.Conv1D(filters=64, kernel_size=3, activation='leaky_relu', padding='same'),
        tf.keras.layers.MaxPooling1D(pool_size=2),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),
        tf.keras.layers.Conv1D(filters=128, kernel_size=3, activation='leaky_relu', padding='same'),
        tf.keras.layers.MaxPooling1D(pool_size=2),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.2),
        tf.keras.layers.Conv1D(filters=128, kernel_size=3, activation='leaky_relu', padding='same'),
    ...
    history = model.fit(train_dataset, training_label, batch_size = 256, epochs = 50, validation_split = 0.2, shuffle = True)
#%%    
